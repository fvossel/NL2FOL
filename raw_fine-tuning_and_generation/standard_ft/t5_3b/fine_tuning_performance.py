...
old_tokens = ['∀', '∃', '∧', '∨', '→', '⊕', '↔', '¬']
new_tokens = ['FORALL', 'EXISTS', 'AND', 'OR', 'IMPLIES', 'XOR', 'IFF', 'NOT']

def prepare_data(example):
    """Bereitet Eingabebeispiele für das Training vor."""
    for old, new in zip(old_tokens, new_tokens):
        example['FOL'] = example['FOL'].replace(old, new)
    return {
        'NL': 'translate English natural language statements into first-order logic (FOL): ' + example['NL'],
        'FOL': example['FOL']
    }
