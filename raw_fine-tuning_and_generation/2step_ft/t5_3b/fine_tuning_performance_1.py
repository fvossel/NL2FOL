...
def prepare_data(example):
    ...
    return {
        'NL': 'extract predicates from English natural language statements for translating into first-order logic (FOL):'
        + example['NL'],
        'PREDICATES': f'Predicates=["'+ ", ".join(sorted(current_predicates)) + '"]\n',
        'FOL': example['FOL']
    }
    ...
    nl_sentence = " ".join(
        [
            ...
            for token in sentence.replace(
                 "extract predicates from English natural language statements for translating into first-order logic (FOL):",
                "",
            ).split()
        ]
    )
